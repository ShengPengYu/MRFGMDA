...
class TrainLossReporter(IOptimizer):
    ...
    def postprocess(self, loss):
        ...
        if value_of_next == 'stop':
            return 'stop'
        file = open('train_loss.txt','a',encoding='utf-8')
        self.cummulative_loss += loss
        if self.iteration == 1:
            self.cummulative_loss = 0
            print("Initial loss after "+str(self.iteration)+":"+str(loss))
            file.write("Initial loss after "+str(self.iteration)+":"+str(loss)+"\n")
            return value_of_next
        
        print("train loss of iteraction "+str(self.iteration)+":"+str(loss))
        file.write("train loss of iteraction "+str(self.iteration)+":"+str(loss)+"\n")
        file.close()
...
class EarlyStopper(IOptimizer):
    ...
    def postprocess(self, loss):
        ...
        if value_of_next == 'stop':
            return 'stop'
        file = open('tested_validation.txt', 'a', encoding='utf-8')
        if self.iteration % self.evaluate_every_n == 0:
            if self.criteria == 'score_validation_data':
                ...
                file.write("Tested validation score at iteration "+str(self.iteration)+". Result: "+str(validation_score)+"\n")
