...
if __name__ == '__main__':
    ...
    # all dataset
    all_triplets = io.read_triplets_as_list(all_path, entities_path, relations_path)
    ...
    result = model.score(all_triplets)

    for index,trip in enumerate(all_triplets):
        enti1 = trip[0]
        enti2 = trip[2]
        rel = trip[1]
        adj_matrix[enti1][enti2] = 1
        indict_matrix[enti1][enti2] = result[index]
	  
		
    adj_matrix = adj_matrix[0:578, 578:]
    indict_matrix = indict_matrix[0:578, 578:]
    indict_matrix[indict_matrix < score_threshold] = 0

    print("The predicted score matrix is:")
    print(indict_matrix)

    print("The relationship adjacency matrix of miRNA-disease is:")
    print(adj_matrix)
    print(np.shape(indict_matrix))

    # return np.array([AUPR, AUC, f1, accuracy, recall, spec, precision])

    print("The model is under evaluation......")
    valu_result = auc.model_evaluate(adj_matrix, indict_matrix)
    print("The performance evaluation result of KDFGMDA model is:")
    print()
    table = PrettyTable()
    table.field_names=['AUPR','AUC','F1_score','accuracy','recall','spec','precision']
    table.add_row(valu_result)
    print(table) 
    print()
    print("The model completes the evaluation.")
